...
type Item = { id:number; title:string; createdAt:string; author?: { id:number; name:string } }

export function ResourcesPage() {
  const [items, setItems] = useState<Item[]>([])
  const [page, setPage] = useState(0)
  const [totalPages, setTotalPages] = useState(0)
  const goNew = () => { window.location.href = '/resources/new' }

  const load = async (p = 0) => {
    const { data } = await api.get('/notices', { params: { category: 'RESOURCE', page: p, size: 10 } })
  ...
  return (
    <div className="container">
      <div className="section" style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <h1 className="card-title" style={{ fontSize: '2rem', marginBottom: 0 }}>자료실</h1>
              ...
              {items.map((n, idx) => (
                <tr key={n.id} style={{ borderBottom:'1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding:'10px 8px' }}>
                    <Link to={`/resources/${n.id}`}>{n.title}</Link>
                  </td>
